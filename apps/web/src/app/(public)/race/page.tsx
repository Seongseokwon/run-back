  ...
  return (
    <div className="space-y-6 pt-2">
      ...
      {months.map(([month, list]) => (
        <section key={month}>
          <h2 className="text-[18px] font-bold text-ink">{monthLabel(month)}</h2>
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-[16px] font-bold text-ink">
                        ...
                        {race.status === 'uncertain' ? (
                          <span className="ml-2 rounded-full bg-surface-sunken px-2 py-0.5 align-middle text-[11px] font-semibold text-ink-muted">
                            개최 미확정
                          </span>
                        ) : null}
                      </p>
      ...
      <section className="rounded-card border border-line bg-surface px-5 py-6 text-center">
        <p className="text-[17px] font-bold text-ink">찾는 대회가 없나요?</p>
        <p className="mt-1 text-[15px] text-ink-muted">날짜를 직접 넣어도 플랜을 만들 수 있습니다.</p>
        <div className="mt-4">
